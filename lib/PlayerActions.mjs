...
// An action belongs to a job when the `ClassJobCategory` column named after
// that job's abbreviation is set. XIVAPI names those columns from its own
// schema, which lags the game: BST's column is still the unnamed `Unknown0`,
// and querying `ClassJobCategory.BST` is a 400. Drop the entry once the schema
// names the column — the build warns if the query stops matching.
...
export default class PlayerActions {
  ...
  async fetchActions({ sheets, query }) {
    const { Abbreviation } = this.job;
    const categoryColumn = categoryColumns[Abbreviation] ?? Abbreviation;
    const baseQuery = {
      ClassJob: this.job.ID,
      'IsPlayerAction': this.isCrafter ? undefined : 1,
      [`+ClassJobCategory.${categoryColumn}`]: 1,
      ...query
    };
